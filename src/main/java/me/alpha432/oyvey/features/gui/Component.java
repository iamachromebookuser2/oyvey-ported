...
public class Component extends Feature {
    ...
    public void drawScreen(DrawContext context, int mouseX, int mouseY, float partialTicks) {
        ...
        int color = ColorUtil.toARGB(
                ClickGui.getInstance().topRed.getValue(),
                ClickGui.getInstance().topGreen.getValue(),
                ClickGui.getInstance().topBlue.getValue(),
                255
        );

        int headerColor = ClickGui.getInstance().rainbow.getValue()
        ...
        if (this.open) {
            ...
            RenderUtil.rect(context.getMatrices(), this.x, this.y + 12.5f, this.x + this.width, this.y + this.height + totalItemHeight, 0x77000000);
        }

        // Draw component name
        drawString(this.getName(), this.x + 3f, this.y - 4f - OyVeyGui.getClickGui().getTextOffset(), -1);

        // Enable scissor to clip child items drawing
        ScissorUtil.enable(context, x, 0, x + width, mc.getWindow().getScaledHeight());

        if (this.open) {
            float y = this.y + this.height - 3f;
            for (Item item : this.items) {
                counter1[0]++;
                if (item.isHidden()) continue;
                item.setLocation(this.x + 2f, y);
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        if (mouseButton == 0 && this.isHovering(mouseX, mouseY)) {
            ...
            OyVeyGui.getClickGui().getComponents().forEach(component -> {
                if (component.drag) component.drag = false;
            });
            this.drag = true;
            return;
        }
    ...
    public boolean isHovering(int mouseX, int mouseY) {
        return mouseX >= x && mouseX <= x + width && mouseY >= y && mouseY <= y + height - (open ? 2 : 0);
    }
